    ...
    public static double evaluate(Vessel v, vessel_info vi, vessel_resources resources, List<string> modifiers)
    {
      ...
      foreach(string mod in modifiers)
      {
        switch(mod)
        {
          ...
          case "addwatts_pos":
            k *= vi.above_ideal_pos_modifier;
            break;

          case "addwatts_neg":
            k *= vi.above_ideal_neg_modifier;
            break;

          case "removewatts_pos":
            k *= vi.below_ideal_pos_modifier;
            break;

          case "removewatts_neg":
            k *= vi.below_ideal_neg_modifier;
            break;
    ...
    public static double evaluate(environment_analyzer env, vessel_analyzer va, resource_simulator sim, List<string> modifiers)
    {
      ...
      foreach(string mod in modifiers)
      {
        switch(mod)
        {
          ...
          case "addwatts_pos":
            k *= va.above_ideal_pos_modifier;
            break;

          case "addwatts_neg":
            k *= va.above_ideal_neg_modifier;
            break;

          case "removewatts_pos":
            k *= va.below_ideal_pos_modifier;
            break;

          case "removewatts_neg":
            k *= va.below_ideal_neg_modifier;
            break;
