...
namespace KERBALISM {
  ...
  public static class Sim
  {
    ...
    public static double EnvTemperature(Vessel v, Vector3d position, double sunlight, double atmo_factor, out double solar_flux, out double albedo_flux, out double body_flux, out double total_flux, out double surface_temperature)
    {
      // get vessel body
      ...
      //Debug.Log("flux (W/m^2) at " + body.name + " : " + total_flux.ToString("F4"));

      // calculate temperature
      surface_temperature = BlackBodyTemperature(total_flux);
      //Debug.Log("surface temp (K) at " + body.name + " : " + surface_temperature.ToString("F4"));
      ...
      if (body.atmosphere && v.altitude < body.atmosphereDepth)
      {
        ...
        //Debug.Log("surface (atmo) temp (K) at " + body.name + " : " + (Lib.Mix(body.GetTemperature(v.altitude), surface_temperature, Lib.Clamp(v.altitude / body.atmosphereDepth, 0.0, 1.0))).ToString("F4"));
        return Lib.Mix(body.GetTemperature(v.altitude), surface_temperature, Lib.Clamp(v.altitude / body.atmosphereDepth, 0.0, 1.0));
      }

      // else return surface temperature
